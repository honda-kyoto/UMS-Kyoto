	...
	new Ajax.Updater("addBelongListJs", 'ajax/addBelongList.php',
					{
						...
						parameters: 'key='+encodeURIComponent(key)+'&tbody_id='+encodeURIComponent(tbody_id)+'&pattern=2'
					});

	////// ↑大分類と部門を非表示にし、職員番号を追加するためのパラメータを追加
}
...
function deleteBelongList(key, obj)
{
	...
	for (var i = key ; i < maxKey ; i++)
	{
		...
		var classObj = document.getElementById(prefix + "belong_class_id");
		classObj.selectedIndex = document.getElementById("sub_" + next + "_belong_class_id").selectedIndex;
		//if (classObj.selectedIndex != 0)
		//{
		//	belongClassChange(classObj.value, prefix);
		//}

		var divObj = document.getElementById(prefix + "belong_div_id");
		divObj.selectedIndex = document.getElementById("sub_" + next + "_belong_div_id").selectedIndex;
		//if (divObj.selectedIndex != 0)
		//{
		//	belongDivChange(divObj.value, prefix);
		//}

		var depObj = document.getElementById(prefix + "belong_dep_id");
		depObj.selectedIndex = document.getElementById("sub_" + next + "_belong_dep_id").selectedIndex;
		//if (depObj.selectedIndex != 0)
		//{
		//	belongDepChange(depObj.value, prefix);
		//}

		var secObj = document.getElementById(prefix + "belong_sec_id");
		secObj.selectedIndex = document.getElementById("sub_" + next + "_belong_sec_id").selectedIndex;
		//if (secObj.selectedIndex != 0)
		//{
		//	belongSecChange(secObj.value, prefix);
		//}
		...
		document.getElementById("sub_" + i + "_staff_id").value = document.getElementById("sub_" + next + "_staff_id").value;
	}
