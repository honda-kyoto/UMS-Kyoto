              ...
              <tr>
                ...
                <td colspan="5"><div class="CheckBoxTab">
                  <table border="0" cellpadding="0" cellspacing="0">
                    <tr>
<?php if ($mgr->getOutputData('invalid_flg') == "1") { ?>
                      <td><input type="checkbox" name="mail_disused_flg_dis" id="mail_disused_flg_dis" value="1" <?php echo $mgr->getCheckData('mail_disused_flg', '1') ?> disabled />
                      <input type="hidden" name="mail_disused_flg" id="mail_disused_flg" value="<?php echo $mgr->getRequestData('mail_disused_flg') ?>" /></td>
                      <td><label for="mail_disused_flg">メールを使用しない</label>
                      <span style="font-size: 12px;color: #f00;padding-bottom: 5px;">※アカウント無効のため変更できません。</span></td>
<?php } else { ?>
                      <td><input type="checkbox" name="mail_disused_flg" id="mail_disused_flg" value="1" <?php echo $mgr->getCheckData('mail_disused_flg', '1') ?> onClick="changeAccText(this);" <?php echo ($mgr->getOutputData('invalid_flg') == "1" ? "readonly" : "") ?>  /></td>
...
<?php } ?>
              ...
              <tr>
                <th>メール<br />アカウント</th>
                <td colspan="5"><input name="mail_acc" type="text" id="mail_acc" size="20" maxlength="30" value="<?php echo $mgr->getRequestData('mail_acc') ?>" <?php echo ($mgr->getRequestData('mail_disused_flg') == "1" ? "disabled" : "") ?> <?php echo ($mgr->getOutputData('invalid_flg') == "1" ? "readonly" : "") ?> />
                  <?php echo USER_MAIL_DOMAIN ?>
<?php if ($mgr->getOutputData('invalid_flg') == "1") { ?>
                  <span style="font-size: 12px;color: #f00;padding-bottom: 5px;">※アカウント無効のため変更できません。</span>
<?php } ?>
                  <br /><span class="inputRule">(数字、英字大文字・小文字、-（ハイフン）、_（アンダースコア）、.（ピリオド）　3～30文字)</span></td>
              </tr>
<?php if ($mgr->getRequestData('user_id') != "") { ?>
              <tr>
                <th>各種通知書</th>
                <td colspan="5">
                  <input type="button" value="　　<?php echo MAIN_SYSTEM_NAME ?>ネット統合ID通知書発行　　" onclick="printNcvcID();">
...
<?php } ?>
          ...
          <table width="100%" border="0" cellpadding="5" cellspacing="0" class="inputTab">
              ...
              <tr>
                ...
                <td colspan="3"><div class="CheckBoxTab">
                    <?php echo $mgr->getCheckBoxList('user_role_id', 5) ?>
                  </div></td>
              </tr>
              <tr>
              ...
              <tr>
                <th width="100">ファイル転送<br />機能利用</th>
                <td colspan="3"><div class="CheckBoxTab">
                  <table border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      <td><input type="checkbox" name="ftrans_user_flg" id="ftrans_user_flg" value="1" <?php echo $mgr->getCheckData('ftrans_user_flg', '1') ?> /></td>
                      <td><label for="ftrans_user_flg">ファイル転送機能を利用する</label></td>
                      <td>&nbsp;</td>
                      <td>
                        <select name="ftrans_user_kbn" id="ftrans_user_kbn">
                          <option value="">--権限--</option>
                          <?php echo $mgr->getSelectList('ftrans_user_kbn') ?>
                        </select>
                      </td>
                    </tr>
                  </table></div>
