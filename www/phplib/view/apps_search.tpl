...
<?php if ($mgr->isNormalUser()) { ?>
        <input type="button" onclick="formSubmit('input', 'apps_entry.php');" value="　　　登録申請　　　">
<?php } else { ?>
        <input type="button" onclick="formSubmit('input', 'apps_add.php');" value="　　　新規登録　　　">
<?php } ?>
      </div>
...
<?php   if (count($mgr->aryList) > 0) { ?>
        ...
        <table width="100%" border="0" cellpadding="2" cellspacing="0" class="listTab">
          <tr>
            ...
            <th scope="col"><a class="sortlink" href="javascript:sort('app_user_name');"><img src="<?php echo $mgr->getOrderData('src', 'app_user_name') ?>" alt="<?php echo $mgr->getOrderData('msg', 'app_user_name') ?>">利用者</a></th>
...
<?php     foreach ($mgr->aryList AS $id => $data) { ?>
          <tr>
            ...
            <td align="center" nowrap><?php echo $data['app_user_name'] ?></td>
<?php if ($mgr->isAdminUser()) { ?>
<?php   if ($data['is_pending_data']) { ?>
            <td align="center">[<a href="javascript:appsPending(<?php echo $id ?>, <?php echo $data['entry_no'] ?>);">詳細</a>]</td>
            <td align="center">-</td>
<?php   } else { ?>
            <td align="center">[<a href="javascript:appsEdit(<?php echo $id ?>);">詳細</a>]</td>
            <td align="center">[<a href="javascript:appsDelete(<?php echo $id ?>);">削除</a>]</td>
<?php   } ?>
<?php } else { ?>
<?php   if ($data['is_pending_data']) { ?>
            <td align="center">[<a href="javascript:appsPending(<?php echo $id ?>, <?php echo $data['entry_no'] ?>);">詳細</a>]</td>
            <td align="center">-</td>
<?php   } else { ?>
            <td align="center">[<a href="javascript:appsDetail(<?php echo $id ?>);">詳細</a>]</td>
<?php     if ($data['is_other_user']) { ?>
            <td align="center">-</td>
<?php     } else { ?>
            <td align="center">[<a href="javascript:appsDelEntry(<?php echo $id ?>);">削除申請</a>]</td>
<?php     } ?>
<?php   } ?>
<?php } ?>
