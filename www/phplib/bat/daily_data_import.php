...
print_r("szk_nme:".$vals['syksy_nme']);
		if ($vals['syksy_nme'] != "")
		{
...
print_r("szk_nme:".$vals['syksy_cde']);
...
print_r("user_id:".$user_id."\n");
		...
		else
		{
			...
			//$ret = $oMgr->oDb->query($sql);
			array_push($sql_userId_update,$sql);

		}

	}

	foreach ($sql_userId_update as &$value){
		print_r($value."\n");
		$mgr = new users_detail_mgr();
		$ret = $mgr->oDb->query($value);
		print_r("ret:".$ret."\n");
