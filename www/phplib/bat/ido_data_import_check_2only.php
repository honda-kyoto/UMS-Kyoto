...
SQL;
			...
			if (is_array($aryRet) && count($aryRet) >= 2)
			{
				...
				//echo $message.PHP_EOL;
				file_put_contents("/var/www/phplib/import/"."sonota_2only.csv", "②".$body, FILE_APPEND);
				continue;
			}
			else if (is_array($aryRet) && count($aryRet) == 1)
			{
...
//var_dump($aryRet);
				...
				//１件一致した場合
					...
					//echo $message.PHP_EOL;
					file_put_contents("/var/www/phplib/import/"."sabun_ari_2only.csv", "②".$change.$body, FILE_APPEND);
				}
				//差分がない場合は、更新しない
				else
				{
					...
					//echo $message.PHP_EOL;
					file_put_contents("/var/www/phplib/import/"."sabun_nashi_2only.csv", "②".$body, FILE_APPEND);
				}
