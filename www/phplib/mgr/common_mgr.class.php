...
class common_mgr extends base
{
	...
	function setUserAdminRole($user_id, $role_id)
	{
		...
		// 存在チェック
		$sql = $this->getQuery('CMN_EXISTS_USER_ROLE', $args);
		...
		if ($del_flg == "1")
		{
			$sql_id = 'CMN_UPDATE_USER_ROLE';
		}
		else
		{
			$sql_id = 'CMN_INSERT_USER_ROLE';
		}
...
//				'proxy_port' => 8080,
	...
	function getCardAry()
	{
		$sql = $this->getQuery('GET_CARD_ARY');
	 ...
	 * 連携エラーログ
	 */
	function makeRelationErrLog($target_id, $class_name, $function_name, $relation_type)
	{
		$this->oDb->begin();

		$args = array();
		$args[0] = $_SESSION['LOGIN_USER_ID'];
		$args[1] = $target_id;
		$args[2] = $class_name;
		$args[3] = $function_name;
		$args[4] = $relation_type;

		$sql = $this->getQuery('MAKE_RELATION_ERR_LOG', $args);
		...
		{
			$this->oDb->rollback();
			Debug_Trace("連携エラーログ作成失敗：[".$sql."]", 9999);
			return;
		}
