...
require_once("sql/post_mst_sql.inc.php");

//----------------------------------------------------------
//	クラス名	：base
//	処理概要	：画面の共通関数クラス
//----------------------------------------------------------
class post_mst_mgr extends common_mgr
{
	...
	*/
	...
	{
		// 新規レコード作成
		$ret = $this->makeNewRec($request['new_post_name']);
	...
	function updateData($request)
	{
		...
		// 既存レコード編集
		$ret = $this->editExistRec($post_id, $request['post_name'][$post_id]);
	...
	function editExistRec($post_id, $post_name)
	{
		// データを作成
		$args = $this->getSqlArgs();
		$args[0] = $post_id;
	...
	function updateAll($request)
	{
		...
		if (is_array($request['post_name']))
		{
			...
			{
				// 既存レコード編集
				$ret = $this->editExistRec($post_id, $post_name);
