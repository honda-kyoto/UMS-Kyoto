...
require_once("sql/card_mst_sql.inc.php");

//----------------------------------------------------------
//	クラス名	：base
//	処理概要	：画面の共通関数クラス
//----------------------------------------------------------
class card_mst_mgr extends common_mgr
{
	...
	*/
	...
	{
		// 新規レコード作成
		$ret = $this->makeNewRec($request['new_post_name'],$request['new_card_uid']);
	...
	function makeNewRec($card_name, $card_uid)
	{
		// データを作成
		$args = $this->getSqlArgs();
		$args[0] = $card_name;
	...
	function updateData($request)
	{
		...
		// 既存レコード編集
		$ret = $this->editExistRec($post_id, $request['post_name'][$post_id], $request['card_uid'][$post_id]);
	...
	function editExistRec($card_id, $card_name, $card_uid)
	{
		// データを作成
		$args = $this->getSqlArgs();
		$args[0] = $card_id;
	...
	function updateAll($request)
	{
		...
		if (is_array($request['post_name']))
		{
			...
			{
				// 既存レコード編集
				$ret = $this->editExistRec($post_id, $post_name, $request['card_uid'][$card_id]);
